<template>
  <div class="dark:text-white geto-search-table">
        ...
        >
          <template v-slot:button-row>
            <el-button
              ...
              v-auth="'OPERATE_ORGANIZATION_BUSINESS_SYNC'"
            >同步数据
            </el-button>
          </template>
    ...
    <SyncDialog
      ref="syncDialogRef"
      :task_func="syncList"
      class="MspHelper\User\Task\OrganizationBusinessListSyncTask"
      @onSearch="onRefresh"
    />
...
const init_data_config = {
  title: "业务组织列表",
  method: "POST",
  url: "/api/msp-helper/user/organization-business/list",
  info_permission: "OPERATE_ORGANIZATION_BUSINESS_INFO",
  export_func: exportList,
  export_permission: "OPERATE_ORGANIZATION_BUSINESS_EXPORT",
...
const onSearch = val => {
  // 接收搜索组件参数
  searchParams.value = Object.assign(searchParams.value, defaultSearchParam.value, val);
  // 传递查询参数进表格
