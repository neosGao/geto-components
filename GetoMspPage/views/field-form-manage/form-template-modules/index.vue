<template>
  <div class="dark:text-white geto-search-table">
    ...
    <Add ref="addRef" @confirm="addconfirm" />
  </div>
</template>
<script lang="ts" setup>
import { ref, onMounted } from "vue";
import {
  formModulesChangeStatus,
  formModulesDelete,
  formModulesConf
...
const init_data_config = {
  title: "表单列表",
  url: "/api/msp-helper/fieldandform/module-config/index",
  method: "POST",
  export_func: "",
...
const getListConf = async () => {
  const res = await formModulesConf();
...
const handleDelete = async () => {
  ...
  const { code, message } = await formModulesDelete({
    id: ids.value[0]
  });
  if (code === "0") {
...
const handleDisbleOrEnable = async status => {
  const { code, message } = await formModulesChangeStatus({
    ids: ids.value,
