<template>
  <div class="dark:text-white geto-search-table">
    <GetoTabs
      :params="view_conf"
      searchAuth="OPERATE_MD_SPARE_PARTS_PRODUCTION_SEARCH"
      @onSearch="onSearch"
      :defaultTab="tabName"
    />
    <GetoTable
    ...
    >
      <template v-slot:button-row>
        ...
        <el-button
          class="ml-1"
          @click="handleSync"
          v-auth="'OPERATE_MD_SPARE_PARTS_PRODUCTION_SYNC'"
        >
          同步
        </el-button>
        <el-button
          ...
          v-auth="'OPERATE_MD_PROJECT_ANALYSIS_SYNC'"
        >
          查看配置
        </el-button>
      </template>
...
const init_data_config = {
  title: "备库件生产单列表",
  url: "/api/msp-helper/businessmasterdata/mes/" + path + "/list",
  method: "POST",
  task_class: "MdScmExcelExport",
  export_func: exportList,
  export_permission: "OPERATE_MD_SPARE_PARTS_PRODUCTION_EXPORT",
  info_permission: "",
  useSelection: true,
  useIndex: true,
