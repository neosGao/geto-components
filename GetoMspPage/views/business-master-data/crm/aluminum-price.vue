<template>
  <div class="dark:text-white geto-search-table">
    <GetoTabs
      :params="view_conf"
      searchAuth="OPERATE_MD_ALUMINUM_PRICE_SEARCH"
      @onSearch="onSearch"
      :defaultTab="tabName"
    />
    <GetoTable
    ...
    >
      <template v-slot:button-row>
        <!-- 按钮位置插槽 -->
        <el-button class="ml-1" @click="handleSync" v-auth="'OPERATE_MD_ALUMINUM_PRICE_SYNC'">
          同步
        </el-button>
        <el-button class="ml-1" @click="handleViewConfig" v-auth="'OPERATE_MD_ALUMINUM_PRICE_SYNC'">
          查看配置
        </el-button>
      </template>
...
import { getListPageConf, exportList, sync, getMdConf } from "./api";
...
const init_data_config = {
  title: "铝单价列表",
  url: "/api/msp-helper/businessmasterdata/crm/" + path + "/list",
  method: "POST",
  task_class: "MdCrmExcelExport",
  export_func: exportList,
  export_permission: "OPERATE_MD_ALUMINUM_PRICE_EXPORT",
  info_permission: "",
  useSelection: true,
  useIndex: true,
...
const getListPageConfAPI = async () => {
  ...
  const default_view: any = view_conf.value.filter((item: any) => item.name === tabName.value)[0];
...
const configDialog: any = ref(null);
const handleViewConfig = () => {
  configDialog.value.show(path);
};
// 挂载
