...
const emits = defineEmits(["getSelectOrganization"]); // 定义发射给父组件的事件
const treeList = ref({
  filterText: "",
...
const tableConf = {
  ...
  cols: [
    ...
    { prop: "long_name", label: "长名称", width: "500" },
  ],
  options: {
    isIndex: true,
    tableHeight: "415px"
...
const show = async (app_code, targetObject = null) => {
  ...
  if (targetObject) {
    curSelect.value.data = targetObject.values.map(i => ({id:i.id, name:i.name, label:i.name, main_code:i.main_code}))
  }
...
const confirmAdd = () => {
  ...
  console.log('getSelectOrganization', curSelect.value.data);
  console.log(curSelect.value.data);

  selectUser.value.visible = false;
  emits("getSelectOrganization", curSelect.value.data);
};
