...
const tableConf = {
  ...
  cols: [
    ...
    { prop: "work_id", label: "工号", width: "100" },
...
const show = async (app_code, targetObject = null) => {
  ...
  if (targetObject) {
    curSelect.value.data = targetObject.values.map(i => ({id:i.id, main_code:i.main_code, name:i.name, work_id:i.work_id, label:i.name + '（' + i.work_id + '）'}))
  }

  const res = await AdminOrgTree({app_code:app_code});
  if (res.code == "0") {
...
const getTableData = async filter => {
  ...
  const res = await UserList({
    ...
    include_next_level: treeList.value.include_next_level ? 1 : 0,
    organization_id: treeList.value.organization_id,
  });
...
const confirmAdd = () => {
  ...
  console.log('getSelectUser', curSelect.value.data);
  console.log(curSelect.value.data);

  selectUser.value.visible = false;
  emits("getSelectUser", curSelect.value.data);
};
...
const emits = defineEmits(["getSelectUser"]); // 定义发射给父组件的事件
</script>
