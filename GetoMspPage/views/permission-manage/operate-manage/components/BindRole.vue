<template>
  <div>
    <el-button plain @click="show" class="ml-1">绑定角色</el-button>
...
const show = async () => {
  ...
  operate_ids.value = cloneDeep(props.ids);
  drawer.value = true;
  resetForm();
  const query = {
    operate_id: 0
  };
  if (operate_ids.value.length == 1) {
    query.operate_id = operate_ids.value[0];
  }
  const res: any = await bindRoleConf(query);
  role_list.value = res.data.un_bind_role_list;
  form.value.role_ids = res.data.en_bind_user_ids;
...
const confirmClick = async () => {
  loading.value = true;
  form.value.operate_ids = operate_ids.value;
