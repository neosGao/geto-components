<template>
  <div class="dark:text-white geto-search-table">
    <GetoTabs
      :params="view_conf"
      searchAuth="OPERATE_PERMISSION_LIST_MANAGE_SEARCH"
      @onSearch="onSearch"
    />
    <GetoTable
      ref="getoTable"
    ...
    >
      <template v-slot:button-row>
        ...
        <SyncBtn
          @onSearch="onSearch"
          v-auth="'OPERATE_PERMISSION_LIST_MANAGE_SYNC'"
        />
      </template>
    </GetoTable>
  </div>
...
const init_data_config = {
  title: "数据列表列表",
  url: "/api/msp-helper/permission/list-manage/list",
  method: "POST",
  task_class: "ListListExcelExportTask",
  export_func: exportList,
  export_permission: "OPERATE_PERMISSION_LIST_MANAGE_EXPORT",
  info_permission: "OPERATE_PERMISSION_LIST_INFO",
  useIndex: true
};
...
const selectionChange = selection => {
  ...
  list_keys.value = selection.map(item => item.list_key);
};
// 挂载
onMounted(() => {
  getListPageConfAPI();
